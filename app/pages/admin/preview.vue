...
const selectedSection = ref<SectionKey>('hero')
const selectedAboutSection = ref<AboutSectionKey>('hero')
const activeTab = ref<'home' | 'header' | 'categories' | 'producto' | 'about' | 'styles' | 'brand' | 'navbar' | 'footer'>('home')
...
<template>
  <div class="h-[calc(100dvh-4rem)] flex flex-col -m-6">
    ...
    <!-- Fullscreen Preview -->
    <div v-if="fullscreen" class="flex-1 flex flex-col">
      <div class="flex items-center gap-2 px-4 py-2 bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
        <span class="text-xs text-slate-500 font-medium">Vista previa</span>
        <div class="flex-1" />
        <UButton size="xs" color="neutral" variant="ghost" icon="i-lucide-minimize-2" label="Salir" @click="fullscreen = false" />
      </div>
      <div class="flex-1 overflow-y-auto bg-white dark:bg-slate-950">
        <AdminSectionPreview :config="config" :preview-page="activeTab === 'about' ? 'nosotros' : undefined" />
      </div>
    </div>
    ...
    <div v-else class="flex-1 flex overflow-hidden">
      ...
      <div class="w-80 border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 flex flex-col shrink-0">
        ...
        <!-- Style editors -->
        <div v-else-if="activeTab === 'styles'" class="flex-1 overflow-y-auto p-4 btn-global">
          <UTabs color="neutral" :items="[
            ...
            <template #navbar class="flex-1 overflow-y-auto p-4">
              <AdminEditorsNavbarEditor
                :value="config.navbar"
                @update="updateNavbar"
              />
            </template>
            <template #footer class="flex-1 overflow-y-auto p-4">
      ...
      <div class="flex-1 overflow-y-auto bg-slate-100 dark:bg-slate-900">
        <div class="bg-white dark:bg-slate-950 min-h-full">
          ...
          <AdminSectionEditor
            ...
            :config="config"
            :selected="selectedSection"
            @update:selected="(selectedSection = $event as SectionKey)"
          />
        </div>
      </div>
    </div>
